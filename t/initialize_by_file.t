...
run {
    ...
    my $v = new Religion::Bible::Regex::Versification($c, $b, $block->decalage);

#    print Dumper $v;
    foreach my $tuple (@{$block->decalage}) {
	$r->parse($tuple->[0]);
	my $rd = $v->decalage($r);
#	print $rd->formatted_normalize  ."=". $tuple->[0] .":". $tuple->[1]."\n";

	is($rd->formatted_normalize, $tuple->[1]);
    }
    # Foreach match abbreviation return the formatted_normalized book
    ...
    # # Foreach key return the formatted_normalized book
    # while ( my ($key, $value) = each(%{$bookconfig->{books}})) {
